...
type ApiGetKubernetesScannersRequest struct {
	ctx context.Context
	ApiService *LookupApiService
	reportersLookupFilter *ReportersLookupFilter
}

func (r ApiGetKubernetesScannersRequest) ReportersLookupFilter(reportersLookupFilter ReportersLookupFilter) ApiGetKubernetesScannersRequest {
	r.reportersLookupFilter = &reportersLookupFilter
	return r
}

func (r ApiGetKubernetesScannersRequest) Execute() ([]ModelKubernetesCluster, *http.Response, error) {
	return r.ApiService.GetKubernetesScannersExecute(r)
}

/*
GetKubernetesScanners Retrieve K8s scanners data

Retrieve all the data associated with k8s scanners

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiGetKubernetesScannersRequest
*/
func (a *LookupApiService) GetKubernetesScanners(ctx context.Context) ApiGetKubernetesScannersRequest {
	return ApiGetKubernetesScannersRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return []ModelKubernetesCluster
func (a *LookupApiService) GetKubernetesScannersExecute(r ApiGetKubernetesScannersRequest) ([]ModelKubernetesCluster, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "LookupApiService.GetKubernetesScanners")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/deepfence/lookup/kubernetes-scanners"
