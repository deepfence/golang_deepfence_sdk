...
> []map[string]string GetAllNodesOfScanResultDocument(ctx, docId, scanId, scanType).Execute()
...
func main() {
    ...
    // response from `GetAllNodesOfScanResultDocument`: []map[string]string
...
## GetScanResultDocument

> map[string]string GetScanResultDocument(ctx, docId, scanId, scanType).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.ScanResultsApi.GetScanResultDocument(context.Background(), docId, scanId, scanType).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ScanResultsApi.GetScanResultDocument``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetScanResultDocument`: map[string]string
    fmt.Fprintf(os.Stdout, "Response from `ScanResultsApi.GetScanResultDocument`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetScanResultDocumentRequest struct via the builder pattern
